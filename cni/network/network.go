...
func (plugin *netPlugin) Add(args *cniSkel.CmdArgs) error {
	...
	if cniConfig.Ipam.Type != "" {
		...
		// _, ipnet, err := net.ParseCIDR("172.210.2.0/24")
		// if err != nil {
		// 	logrus.Errorf("[cni-net] error:%s", err.Error())
		// }
		// var subnetInfo = network.SubnetInfo{
		// 	AddressPrefix:  *ipnet,
		// 	GatewayAddress: net.ParseIP("172.210.2.1"),
		// }
		// networkInfo.Subnets = append(networkInfo.Subnets, subnetInfo)
		// epInfo.IPAddress = net.ParseIP("172.210.2.2")
		// epInfo.Gateway = net.ParseIP("172.210.2.1")
		// epInfo.Subnet = *ipnet
	}

	logrus.Infof("networkInfo %+v", networkInfo)
	logrus.Infof("epInfo %+v", epInfo)
